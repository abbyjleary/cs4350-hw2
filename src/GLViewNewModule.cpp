...
void GLViewNewModule::onKeyDown( const SDL_KeyboardEvent& key )
{
   ...
   std::cout << "you pressed " << SDL_GetKeyName(key.keysym.sym) << std::endl;
}
...
void Aftr::GLViewNewModule::loadMap()
{
   ...
   {
       WO* wo = WO::New(shinyRedPlasticCube, Vector(2, 2, 2));
       wo->setPosition(Vector(-1, -6, 4));
       wo->renderOrderType = RENDER_ORDER_TYPE::roOPAQUE;
       worldLst->push_back(wo);
